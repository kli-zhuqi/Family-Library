...
If you do not want to use Docker, install DeepTutor as its own local app instead. DeepTutor's upstream non-Docker flow requires **Python 3.11+** and **Node.js 20+** on your Windows PATH:
...
py -3.11 -m venv .venv
.\.venv\Scripts\Activate.ps1
python -m pip install -U deeptutor
deeptutor init
deeptutor start
```

After `deeptutor start`, open the frontend URL it prints; by default it is `http://127.0.0.1:3782`.

If you run Ollama, LM Studio, llama.cpp, vLLM, or Lemonade on your host, use `host.docker.internal` in DeepTutor's **Settings → Models** because `localhost` inside Docker points at the container. For the non-Docker DeepTutor install, use your normal local host URLs such as `http://127.0.0.1:11434` for Ollama.
